  ...
  const ResultCard = ({ result }) => (
    <div className="h-full bg-background flex flex-col article-card rounded-xl border border-black p-5 transition-all duration-500 ease-in-out hover:-translate-y-1 hover:shadow-lg">
      ...
      <Link href={result.link} onClick={onClose} className="grow">
        <h3 className="article-title local-font-rachana text-[25px] h-[70px] overflow-hidden font-bold leading-[22px] py-1 text-red-600 hover:text-red-700 transition-colors">
          {result.title}
        </h3>
      </Link>
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <form onSubmit={handleFormSubmit} className="mb-16">
          <div className="flex items-center border border-black rounded-[50px] bg-background p-2.5">
            <input
              ...
              className="font-poppins text-2xl w-full bg-transparent outline-none px-10 py-2.5 placeholder:text-black/30"
        ...
        {results.length > 0 && (
          <div className="text-center mt-12">
            <button className="bg-red-600 text-white font-bold py-3 px-12 rounded-full border border-black transition-all duration-300 hover:bg-red-700 shadow-xl">
              SEE MORE
            </button>
          </div>
        )}

...
const RichTextEditor = ({ value, onChange, onImageUpload }) => {
  const [modals, setModals] = useState({ link: false, image: false, table: false, find: false, footnote: false });
  const [editingFootnoteNode, setEditingFootnoteNode] = useState(null);
  ...
  const handleShowFootnote = () => {
    saveSelection();
    setEditingFootnoteNode(null);
    setModals(prev => ({ ...prev, footnote: true }));
  };

  const insertFootnote = (text) => {
    setModals(m => ({ ...m, footnote: false }));
    restoreSelection();
    
    const escapedText = escapeHtml(text);
    const footnoteHtml = `<span class="footnote-ref-marker" data-note="${escapedText}" contenteditable="false"></span>&nbsp;`;
    execCommand("insertHTML", footnoteHtml);
  };

  const updateFootnote = (text) => {
    setModals(m => ({ ...m, footnote: false }));
    if (editingFootnoteNode) {
      editingFootnoteNode.setAttribute('data-note', text);
      setEditingFootnoteNode(null);
      handleInput();
    }
  };
    ...
    <div className="w-full flex flex-col bg-slate-50 border border-slate-200 rounded-lg overflow-hidden">
      ...
      <div className="flex-1 overflow-auto p-4 bg-slate-50">
        <div 
          ...
          onClick={(e) => {
            const marker = e.target.closest('.footnote-ref-marker');
            if (marker) {
              e.preventDefault();
              e.stopPropagation();
              setEditingFootnoteNode(marker);
              setModals(m => ({ ...m, footnote: true }));
            } else {
              checkActiveCommands();
            }
          }}
      ...
      <FootnoteModal
        ...
        onInsert={editingFootnoteNode ? updateFootnote : insertFootnote}
        editingNode={editingFootnoteNode}
      />
    </div>
  );
};

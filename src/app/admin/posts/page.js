...
export default function PostsPage() {
  ...
  }, [addNotification, loadData, currentPage]);

  const updatePostProperty = useCallback(async (post, update) => {
    try {
      const res = await fetch("/api/admin/posts", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSendNewsletter = useCallback(async (postId) => {
  ...
  ], [settings, handleToggleStatus, handleSendNewsletter, handleEdit, handleCopyLink, handleDelete]);
